...
schedule and status as an array instead of a string; verify that schdule hour is >0 and <24, minutes between 0 and 59
'''
...
class MariaWrapper():
    ...
    def initDeviceTypes (self):
        self.insertDeviceTypes (DEVICE_TYPE_SEED)
        return ({'result' : 'success'}) 

        
    def insertDeviceTypes (self, jdev_types):
    ...
    def initRoomTypes (self):
        self.insertRoomTypes (ROOM_TYPE_SEED)
        return ({'result' : 'success'}) 

        
    def insertRoomTypes (self, jroom_types):
